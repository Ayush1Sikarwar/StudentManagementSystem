package com.tech.StudentManagemet;
...
public class StudentManagementApp {
	...
	static final String DB_PASSWORD = "Ayush@SQL1";
	...
	// escape single quote by doubling it (simple)
	...
	static boolean existsStudent(String id) throws SQLException {
		String sql = "SELECT 1 FROM students WHERE student_id = '" + esc(id) + "'";
	...
	static void addStudent() throws SQLException {
		...
		if (name.length() == 0 || course.length() == 0) {
			System.out.println("Name and Course required");
			return;
		}

		String sql = "INSERT INTO students(student_id, name, course) VALUES ('" + esc(id) + "', '" + esc(name) + "', '"
				+ esc(course) + "')";
	...
	static void updateStudent() throws SQLException {
		...
		if (name.length() == 0 || course.length() == 0) {
			String q = "SELECT name, course FROM students WHERE student_id = '" + esc(id) + "'";
		...
		String sql = "UPDATE students SET name = '" + esc(name) + "', course = '" + esc(course)
				+ "' WHERE student_id = '" + esc(id) + "'";
		try (Statement st = conn.createStatement()) {
			st.executeUpdate(sql);
		}
	}
	...
	static void deleteStudent() throws SQLException {
		...
		String sql = "DELETE FROM students WHERE student_id = '" + esc(id) + "'";
		try (Statement st = conn.createStatement()) {
			st.executeUpdate(sql);
		}
	}
	...
	static void addResult() throws SQLException {
		...
		System.out.print("Marks1 (0-100): ");
		String s1 = sc.nextLine();
		Integer m1 = parseMark(s1);
		if (m1 == null) {
			System.out.println("Invalid mark");
			return;
		}
		System.out.print("Marks2 (0-100): ");
		String s2 = sc.nextLine();
		Integer m2 = parseMark(s2);
		if (m2 == null) {
			System.out.println("Invalid mark");
			return;
		}
		System.out.print("Marks3 (0-100): ");
		String s3 = sc.nextLine();
		Integer m3 = parseMark(s3);
		if (m3 == null) {
			System.out.println("Invalid mark");
			return;
		}
	...
	static void viewResults() throws SQLException {
		...
		try (Statement st = conn.createStatement(); ResultSet rs = st.executeQuery(sql)) {
			boolean any = false;
			System.out.printf("%-12s %-4s %-4s %-4s %-6s %-10s %-5s%n", "StudentID", "M1", "M2", "M3", "Total",
					"Percentage", "Grade");
			while (rs.next()) {
				any = true;
				System.out.printf("%-12s %-4d %-4d %-4d %-6d %-10.2f %-5s%n", rs.getString(1), rs.getInt(2),
						rs.getInt(3), rs.getInt(4), rs.getInt(5), rs.getDouble(6), rs.getString(7));
			}
			if (!any)
	...
	static void updateResult() throws SQLException {
		...
		String sql = "UPDATE results SET marks1 = " + m1 + ", marks2 = " + m2 + ", marks3 = " + m3 + ", total = "
				+ total + ", percentage = " + perc + ", grade = '" + esc(grade) + "' WHERE student_id = '" + esc(sid)
				+ "'";
		try (Statement st = conn.createStatement()) {
			st.executeUpdate(sql);
		}
